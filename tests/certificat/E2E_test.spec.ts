...
test("E2E test: registrace, login FE, vytvořit účet API, vyplnit profil a odhlásit se", async ({
  page,
}) => {
  const profileData = {
    username: faker.internet.userName(),
  ...
  await test.step("Vytvořit účet přes API", async () => {
    const initialBalance = 10000;
    const createAccountResponse = await userApi.createAccount(
      accessToken,
      initialBalance
  ...
  await test.step("Login na FE", async () => {
    ...
    await page.waitForTimeout(500); // bez této pauzy to na GA padá

    // Filtrovaná data bez password pro fillProfile, protoze password na formulari neni
    const profileDataForFill = {
    ...
    await dashboard.fillProfile(profileDataForFill);
    ...
    await dashboard.expectProfileData(profileDataForFill);
